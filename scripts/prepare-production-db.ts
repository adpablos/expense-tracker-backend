...
async function prepareProductionDB() {
  ...
  try {
    ...
    let household = (
      await client.query("SELECT id FROM households WHERE name = 'Diana-Lua-Alex home'")
    ).rows[0];
    ...
    async function createOrGetUser(email: string, name: string, authProviderId: string) {
      let user = (
        await client.query('SELECT id FROM users WHERE auth_provider_id = $1', [authProviderId])
      ).rows[0];
    ...
    const ownerUser = await createOrGetUser(
      'adpabloslopez@gmail.com',
      'Alex',
      'auth0|66bcf68d2b9b21bece891d55'
    );
    ...
    const memberUser = await createOrGetUser(
      'edrada2000@yahoo.es',
      'Diana',
      'auth0|66bcf63d2b9b21bece891d26'
    );

    // Asignar roles en el hogar (si no existen ya)
    const existingMembers = (
      await client.query('SELECT user_id, role FROM household_members WHERE household_id = $1', [
        household.id,
      ])
    ).rows;
    ...
    const updatedCategories = await client.query(
      `UPDATE categories SET household_id = $1 WHERE ${updateQuery} RETURNING id`,
      [household.id]
    );
    const updatedSubcategories = await client.query(
      `UPDATE subcategories SET household_id = $1 WHERE ${updateQuery} RETURNING id`,
      [household.id]
    );
    const updatedExpenses = await client.query(
      `UPDATE expenses SET household_id = $1 WHERE ${updateQuery} RETURNING id`,
      [household.id]
    );
    ...
    if (!isProduction) {
      await client.query(
        "DELETE FROM households WHERE id = '00000000-0000-0000-0000-000000000000'"
      );
