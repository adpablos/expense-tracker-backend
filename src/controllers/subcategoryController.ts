...
export class SubcategoryController {
    constructor(
        @inject(TYPES.SubcategoryService) private subcategoryService: SubcategoryService
    ) {}

    public getSubcategories = async (req: Request, res: Response, next: NextFunction) => {
        try {
            const subcategories = await this.subcategoryService.getAllSubcategories(req.currentHouseholdId);
    ...
    public updateSubcategory = async (req: Request, res: Response, next: NextFunction) => {
        try {
            const {id} = req.params;
            const {name, categoryId} = req.body;
            const updatedSubcategory = await this.subcategoryService.updateSubcategory(id, name, categoryId, req.currentHouseholdId);
