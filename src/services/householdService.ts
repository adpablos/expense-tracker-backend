...
export class HouseholdService {
    ...
    async createHousehold(household: Household, user: User): Promise<Household> {
        ...
        try {
            ...
            const existingUser = await client.query('SELECT * FROM users WHERE auth_provider_id = $1', [user.authProviderId]);
            ...
            throw new AppError(`Error creating household: ${error.message}`, 500);
        } finally {
            client.release();
        }
    }
